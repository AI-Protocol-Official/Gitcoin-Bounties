...
const {ROLE_TOKEN_CREATOR} = require("./include/ali_erc20_features_roles");
		...
		describe("Gas Consumption", function() {
			...
			function consumes_no_more_than(gas) {
				...
				it(`consumes no more than ${gas} gas  [ @skip-on-coverage ]`, async function() {
					const gasUsed = this.receipt.receipt? this.receipt.receipt.gasUsed: this.receipt.gasUsed;
			...
			function gas_usage_suite() {
				gas_usage_deployment(3336431);
				gas_usage_approvals(48520, 48876, 31830, 57389, 79095);
				describe("when delegation is not involved", function() {
					gas_usage_transfers(61696, 71647, 64483, 68779, 78723, 78723, 77179, 77218);
					gas_usage_mint_burn(91302, 76568, 86060, 78940);
				});
				describe("when first address is a delegate", function() {
					beforeEach(async function() {
						await ali.delegate(from, {from});
					});
					gas_usage_transfers(94834, 104782, 97621, 101914, 111858, 111858, 110327, 110366);
				});
				describe("when second address is a delegate", function() {
					beforeEach(async function() {
						await ali.delegate(to, {from: to});
					});
					gas_usage_transfers(108943, 118891, 111730, 68779, 78723, 78723, 124424, 124463);
				});
				describe("when delegation is fully involved", function() {
					...
					gas_usage_transfers(142005, 151953, 144792, 101914, 111858, 111858, 157498, 157537);
					gas_usage_mint_burn(138549, 109706, 119198, 112078);
				});
				describe("when there is nothing on the balances", function() {
					gas_usage_delegation(50691, 80586);
				});
				describe("when one of the balances is non-zero", function() {
					beforeEach(async function() {
						await ali.transfer(by, value.muln(2), {from});
					});
					gas_usage_delegation(97936, 127816);
				});
				describe("when both balances are non-zero", function() {
					...
					gas_usage_delegation(113898, 143790);
				});
			}
