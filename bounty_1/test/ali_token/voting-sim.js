...
// Following simulation executes a lot of token transfers and voting delegation operations,
...
contract("AliERC20: Voting Delegation Simulation", function(accounts) {
	...
	// a – rest of the accounts
	const a = accounts.slice(3);
	...
	async function voting_sim_test(cycles = 1000, p1 = 0.015, p2 = 0.015) {
		...
		const n = a.length;
		// split S0 randomly between the accounts. generate n random numbers [0, S0]
		const p = new Array(n);
		for(let i = 0; i < n; i++) {
			p[i] = random_bn(0, S0);
		}
		// and sort them: now each number represents cumulative balance of the accounts up to account `i`,
		// last account balance is assumed to be S0
		p.sort((a, b) => a.cmp(b));
		// derive the initial balances array:
		const ali_amounts = p.map((v, i, p) => i === p.length - 1? S0.sub(v): p[i + 1].sub(v));
		const total_amount = sum_bn(ali_amounts);

		// transfer the balances from H0 to the target accounts
		for(let i = 0; i < n; i++) {
			await ali.transfer(a[i], ali_amounts[i], {from: H0});
		}
		...
		async function delegate(from_idx, to_idx) {
			...
			const result = await ali.delegate(to_idx < n? a[to_idx]: ZERO_ADDRESS, {from: a[from_idx]});
		...
		async function transfer(from_idx, to_idx, value) {
			// Truffle result: https://www.trufflesuite.com/docs/truffle/getting-started/interacting-with-your-contracts#processing-transaction-results
			const result = await ali.transfer(a[to_idx], value, {from: a[from_idx]});
		...
		async function prob_transfer(p = p2) {
			...
			for(let i = 0; i < n; i++) {
				...
				const balance = await ali.balanceOf(a[i]);
				if(balance.isZero()) {
					// nothing to transfer
					continue;
				}
		...
		async function verify_state(round, state) {
			...
			for(let i = 0; i < n; i++) {
				const balance = await ali.balanceOf(a[i]);
				const voting_power = await ali.votingPowerOf(a[i]);
				const delegate = await ali.votingDelegates(a[i]);
				...
				expect(delegations[i] < 0? ZERO_ADDRESS: a[delegations[i]]).to.equal(
					delegate,
