...
async function eip712_domain(ali_address) {
	...
	// build the domain
	return {name: CONTRACT_NAME, chainId, verifyingContract: ali_address};
}

// helper function to EIP712 sign any of the auth-like ALI token call
async function eip712_sign(ali_address, fn_name, params, p_key) {
	return EIP712.sign(await eip712_domain(ali_address), fn_name, params, ALI_EIP712_TYPES, p_key);
}

// helper function to build EIP712 Delegation signature
async function eip712_delegate(ali_address, delegate, nonce, expiry, p_key) {
...
async function eip712_permit(ali_address, owner, spender, value, nonce, deadline, p_key) {
	return eip712_sign(
		ali_address,
		'Permit',
...
async function eip712_transfer(ali_address, from, to, value, validAfter, validBefore, nonce, p_key) {
	return await eip712_sign(
		ali_address,
		'TransferWithAuthorization',
...
async function eip712_receive(ali_address, from, to, value, validAfter, validBefore, nonce, p_key) {
	return await eip712_sign(
		ali_address,
		'ReceiveWithAuthorization',
		{from, to, value, validAfter, validBefore, nonce},
...
async function eip712_cancel(ali_address, authorizer, nonce, p_key) {
	return await eip712_sign(
		ali_address,
		'CancelAuthorization',
		{authorizer, nonce},
